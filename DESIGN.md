...
- **Shortcuts** (all configurable in config UI, registered as `QShortcut` on the main window). Proposed defaults:
  - **`Cmd+J` / `Ctrl+J` — toggle chat focus**: if dock collapsed to the rail → expand (animated) + focus input; if focus in dock → return focus to reviewer/deck browser; if focus elsewhere and dock expanded → focus input. One chord, three context-aware behaviors. Home-row, one-handed, unused by stock Anki (reviewer, deck browser, or editor). The webview forwards the chord back to Python when the input has focus (Qt shortcuts don't fire inside webviews reliably — known sharp edge).
