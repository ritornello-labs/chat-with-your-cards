...
- **Permission UX (#26, 2026-07-31; unified 2026-08-05)** — collection permission and harness environment power remain separate enforcement axes, but no longer masquerade as two unexplained neighboring chips. One **Access · collection/computer** trigger opens an overview with **Anki collection** and **Computer tools** rows, then drills into either axis. Choice lists are compact labels only; one shared description reflects the selected/focused item, avoiding the clipped wall of repeated hints seen in the old collection ladder. The collection detail retains "What happens under X?" and its per-class matrix (Reads / New notes / Note edits & bulk sweeps / Cards, tags & scheduling / Decks & structure / Grading / Deletes / Skill updates). Presentation only — the carve-outs stay hardcoded in Python; the matrix just stops them being invisible. **Fact correction recorded:** the task's claim that deck & structure ops never auto-apply under trusted-writes was WRONG — `_deck_op_proposal` has flowed through `_finish_submission`'s trusted auto-apply since deck ops were introduced (028aedd, 2026-07-06), verified live; only deletes (always confirmed + backup, and only offered under trusted) and skill updates (always confirmed) are exempt. The copy now describes that reality.
