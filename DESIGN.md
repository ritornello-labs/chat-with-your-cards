...
- **Composer attachments, half (a) (#15a, 2026-07-31)** — an Attach chip next to Pins (same "context riding the next message" category) opens a **native QFileDialog**; picked files are validated and staged through the same media_staging rules (one pseudo-proposal dir per file, so chips remove individually; the startup sweep is the backstop), the chips row shows name/kind/size (paths never reach the UI), and at send the message gains a `<user-attachments>` block listing the staged **paths** with guidance to use `propose_note`'s `media[]` / `store_media_asset`. Files SURVIVE the send (the agent may propose a turn later); new chat discards them. **Deliberately not built yet:** paste/drag-drop (file bytes would have to cross the webview bridge — chunking design needed) and half (b), images/PDFs as agent *context* (multimodal stream-json input blocks; PDF document-block support still unverified) — recorded here, task re-scoped.
...
- **Batched filtered-deck actions (#27 quick win, 2026-07-31)** — `filtered_deck_action` accepts one `deck`, a `decks` list, or a `pattern` glob over filtered-deck names; ONE review card for the whole batch, per-deck gather counts as explicit outcomes on the resolved card (never one opaque success), a normal deck in an explicit list is a loud error while a pattern silently skips what it sweeps up. This directly fixes the lived 50-rebuilds-50-cards case (2026-07-23). **The generic batch machinery (#27 proper) remains open**: generalizing change sets to arbitrary atomic ops with per-item risk classes, per-item revertibility declarations shown up front, single-undo-step semantics, and per-item expand/reject in the React card — deliberately not attempted as a tail-of-session change to the safety-critical apply path.
