...
Status: implemented; v0.1.0 public-preview release candidate. Last updated: 2026-08-06.
...
This document records the implemented architecture, historical decisions, remaining backend work, and known flaws/risks.
...
## 2. Backend strategy: official CLI harnesses

**Decision update (2026-08-06, user-confirmed): the add-on is harness-based, period.** v0.1.0 supports the user's officially installed and authenticated **Claude Code** CLI. **Codex** and **Pi** (`@earendil-works/pi-coding-agent`) remain future harness adapters. CWYC does not accept, resolve, store, or inject API keys.
...
- **Pi** (future): modes `-p`, `--mode json` (event stream: `agent_start/turn_*/message_update{text_delta,toolcall_delta}/tool_execution_*/agent_end`), `--mode rpc` (JSONL over stdio with session resume/fork). **No built-in MCP in the original probe** — re-check before implementation; otherwise the adapter needs a small pi extension bridging tool calls to the add-on's MCP HTTP endpoint. Authentication remains the harness's responsibility.
...
An earlier design pass considered a direct-provider API backend. That option is closed: it duplicated the harness agent loop, degraded executable Agent Skills, expanded the credential and billing support surface, and did not fit the product. There is no API-key or direct-API milestone.

### Why harnesses

1. **The agent loop comes with the harness.** Claude Code, Codex, and Pi implement multi-turn tool use, context management, retries, streaming, model selection, and session resume.
2. **Collection tools stay backend-neutral.** The add-on exposes one registry and each harness adapter connects it through the transport that harness supports.
3. **Agent Skills remain executable.** A harness can use full skills with scripts and assets rather than reducing them to prompt text.
4. **Authentication remains the harness's responsibility.** CWYC neither handles credentials nor invents a second login or billing surface.
...
- The **tool registry is backend-neutral**: plain Python functions with JSON-schema signatures. Each supported harness adapter exposes the same registry through the transport that harness supports.
- The UI is harness-neutral. v0.1.0 selects Claude Code; a future adapter selector can remain per-profile without changing the UI contract.
...
│  Qt main window                                             │
│  ├─ QDockWidget "Chat"                                      │
│  │   └─ AnkiWebView (prebuilt React/CSS bundle)             │
...
│  │   └─ ChatBackend                                         │
│  │        ├─ ClaudeCliBackend ── subprocess: claude         │
│  │        └─ future Codex / Pi harness adapters             │
...
`deck_tree` / `tag_tree` tools always return complete trees on demand. For choosing relevant tags/decks from a large list, the standing instructions recommend the **subagent pattern**: a harness agent spawns a subagent to read the full tree and pick relevant nodes, keeping the main context clean. An LLM reading the actual list beats keyword or embedding search at this scale — no embedding infrastructure is planned.

## 5. Tools

Backend-neutral registry; names and schemas remain identical across harness adapters.
...
- **Statistics blind spots closed (#5, 2026-07-30)** — before these, the agent could see *structure* (decks, tags, note types) but not *performance*, and questions like "what's my retention?" got guesses. All read-only, live SQL/backend calls on the real collection (not the stats cache), measured against a 287k-card / 1.06M-revlog collection so every budget below is empirical:
  - `get_study_stats(days?, deck?)` — true retention (young/mature/total pass rates, review-kind answers only, mature = lastIvl ≥ 21d), answer-button breakdown (learning/young/mature/filtered), review counts by revlog kind + study days + time, card counts by state (incl. suspended/buried), interval percentiles, and avg FSRS stability/difficulty via `json_extract` on `cards.data`. Deck scope includes children and maps filtered-deck cards to their **home** deck (`coalesce(nullif(odid,0), did)`) so a cram session doesn't make cards vanish from their real deck's numbers. ~165ms whole-collection.
  - `get_deck_due_counts(prefix?, include_empty?)` — flattened `col.sched.deck_due_tree()`: exactly the deck browser's remaining-today counts (limits applied, parents include children) plus uncapped totals. ~31ms.
  - `get_due_forecast(days?, deck?)` — raw due dates per day ahead (queues 2/3, ignoring daily limits) plus the overdue backlog. <5ms.
  - `get_card_history(card_id, limit?)` — `col.card_stats_data`: the card-info dialog's numbers including per-review FSRS memory state and top-level stability/difficulty/retrievability. ~18ms.
  - `find_duplicates(field, search?)` — `col.find_dupes`, groups capped at 50. Works only inside real Anki (the bare library crashes on it — i18n/hooks), so the GUI smoke is its only automated proof.
  - `check_media()` — `col.media.check()`; the one slow call (~1.5s+ main-thread block on a large collection), its description says to run it on request, not routinely, and the result warns that 'unused' files are candidates only (dynamically-built names look unused), never auto-deletable.
...
- **Bulk tags (#4, 2026-07-30)** — `add_tags` / `remove_tags` (Browse's Add/Remove Tags; `remove_tags(tags=['X'], query='tag:"X"')` is the delete-a-tag idiom) plus `clear_unused_tags` (registry-only, not revertible, warned as harmless). **No wildcards** — probed against the real 25.09 backend (2026-07-31): `bulk_remove` silently ignores `*` (the Docker smoke caught the assumption before it shipped), and its real semantics are exact case-insensitive names **plus hierarchy descent** (removing `X` also removes `X::child`), which the tools document and the no-op accounting mirrors; `*` in any tag is rejected at submit so it can never no-op while looking accepted. Same selection contract as the card-state ops: exactly one of `note_ids` / `query`, one proposal card, no-op accounting. Revert restores each note's **full prior tag list** (captured at apply) via update_note — recording "what to reverse" would mean reimplementing the backend's wildcard matching, so an exact restore is both simpler and safer; learning snapshots are resynced so the restore is not read as a user edit. The other half of the gap: `search_notes` gained find_cards' `detail`/`limit`/`offset` contract, and both search tools now page `detail='ids'` up to **1000** ids (`MAX_IDS_LIMIT`) while full summaries stay capped at 100 — the flat 100-cap meant "tag every note matching this search" could not even enumerate its targets.
- **Chrome parity (#23, 2026-08-01; composer hierarchy revised 2026-08-05)** — the documented classic-UI gaps closed: (a) the controller still pushes **context state** (card vs collection) for context-aware suggestions and send-time assembly, but its passive composer chip was removed on dogfood feedback — "collection (tools on demand)" repeated the empty state, did nothing on click, and looked interactive; (b) **suggested-question ghost text** — rotating, context-aware placeholders (card vs collection pools) with **Tab-to-accept** on an empty composer, gated by the `suggested_questions` config; (c) the **bulk bar** — two or more pending proposals surface "N proposals pending · Reject all / Accept all" above the composer (bare accepts; Python falls back to each proposal's own values); (d) the **learning nudge** (`learning` pushes) now sits above the writing surface as "N of your edits to learn from · Review patterns", explicitly distinguishing past user corrections from pending agent proposals; (e) **friendly tool labels** — raw MCP names map to human phrases ("Searched your cards"), unknown names prettify, plumbing (ToolSearch, StructuredOutput, TodoWrite) renders nothing, and the raw name stays inside the expanded details so debugging loses nothing.
- **Bulk authoring (#11, 2026-08-01)** — `preview_csv_import` (Anki's own metadata detection: delimiter, HTML flag, notetype/deck mapping, first rows) and `import_csv_file` through Anki's REAL pipeline (`get_csv_metadata`/`import_csv`) behind one review card: ≈row count + preview rows at review, exact created/updated/duplicate/conflicting counts on the resolved card. **Safety divergence from Anki, deliberate:** `existing_notes` defaults to `preserve` (Anki's own default is Update, which rewrites notes matched on the first field — here Update is opt-in and warned in capitals). Revert removes the CREATED notes (studied ones are kept and reported by name-count; updated notes only come back via the checkpoint backup — declared on the card). Exports: `export_csv` (Anki's re-importable text format) and `export_apkg` (deck/query/collection scope; `legacy=True` for compatibility; the filtered-decks-don't-survive caveat rides every result) — file writes only, read-class like `create_backup_now`. `.colpkg` import stays deliberately human-only.
- **GUI handoff (#12, 2026-08-01)** — `open_browse` opens Anki's real Browse window at a query (validated FIRST, with the match count reported — a typo'd search is a tool error, not a Browse window showing an Anki error), for the "show me those cards" handoff. `list_saved_searches` + `manage_saved_search` (save/update/delete, prior-value revert) manage Browse's sidebar saved searches in collection config — the curricula shipping vehicle per the workspace's APKG lesson (filtered decks do not survive `.apkg` export; saved searches do).
- **Deck & preset management (#9, 2026-08-01)** — `delete_deck` (a card-carrying normal deck NEVER auto-applies, even under trusted — mirrored on `delete_notes` — with a critical backup at apply and an all-caps card-count warning; filtered decks are the mild case, cards return home), `manage_options_preset` (create / clone / rename / delete — delete drops its decks to Default and **forces a one-way sync**, warned with a `get_sync_status` pointer; revert *recreates* the preset from a captured snapshot and reassigns the affected decks, the forced sync notwithstanding), `assign_options_preset` (with `include_subdecks` = Save to All Subdecks; prior-preset revert) and `set_deck_description` (prior-desc revert). `set_deck_options`' shared-preset warning now names the correct fix (clone + assign) instead of only warning.
- **Safety net (#8, 2026-08-01)** — `get_undo_status` (labels only), `undo_last_change` (the card names EXACTLY what the queue head would undo — possibly the user's own action — and apply **re-inspects** label + `last_step`: a moved queue is a clean "ask again" error, never a misfire; the inverse is Anki's own redo), `check_database` (`fix_integrity`, checkpoint first, full report on the resolved card, honestly non-revertible), `get_sync_status` (logged-in / normal-vs-FULL sync / pending note+card change counts — so schema-changing proposals can warn "this forces a full upload and you have N pending changes" in advance) and `sync_now` (proposal-gated; Anki's own sync window takes over; the sync starter is injected by the glue so proposals.py stays aqt-free), plus `create_backup_now` (available even in read-only mode — a backup file write touches nothing in the collection). Backup-before-bulk needed **no new work**: every bulk/scheduling/card-state/tag/deck write already checkpoints through `_apply_write(backup_reason=…)` → `_backup_checkpoint`, synchronous for critical (delete) writes — recorded here as the answer to the task's "generalise the delete-notes backup" item.
- **Generic batches (#27 proper, 2026-07-31)** — change sets generalized to arbitrary atomic operations by **composition**, not an op-registry rewrite: `add_to_change_set` takes `{op, args}` items from a curated `BATCHABLE_OPS` allowlist (card-state ×5, tags ×2, scheduling ×3, deck limits, filtered actions), each declaring a **risk class** and an **honest per-item revertibility label** shown up front (undo-UX decision 2026-07-23); light validation at add, full validation through each op's own submit path at apply. The review card lists every item with include-checkboxes (per-item reject → `excluded_items` indices at accept), the batch inherits its highest risk class as a warning, and non-revertible items are named before accept. Apply: ONE checkpoint up front, then each item through `_internal_apply_op` (the op's real submit+accept run in **quiet mode** — pushes, per-item checkpoints and trusted auto-apply suppressed), explicit per-item outcomes (a failure reports and does not strand the rest), native undo entries merged best-effort via `col.merge_undo_entries` (degrades with a stated warning), and every item's revert data scooped into **one `batch` ledger row** (ops that leave no revert data get a named tombstone). Revert walks subs in reverse order through their own op-class paths, reporting non-revertible and failed items by name. Budget draws per ITEM at close (pre-existing).
- **Composer attachments finished (#15, 2026-08-01)** — the deferred halves landed. **Transports:** OS **drag-drop** is intercepted at the Qt layer (event filter on the dock webview + its render child): file URLS become paths, so no bytes cross the bridge, and non-file drags (text) pass through untouched; **paste** is the one transport where bytes legitimately cross (a clipboard screenshot has no path) — image clipboard items go over the bridge as a single data-URL message, the same route Anki's own editor paste uses, capped client-side (8 MB, with a visible notice) and again at staging. **Agent context (#15b):** attached images ≤3.75 MB ride the send as base64 **image content blocks** appended after the text (backend `send` grew an `extra_blocks` param through the ChatSession protocol; the scripted backend accepts and ignores them), and the `<user-attachments>` text says which images are inline vs path-only. **PDFs** stage as a new `document` kind — accepted by the composer and `store_media_asset`, REJECTED by note proposals (a PDF can't render on a card) — and are handled per the task's fallback: path + explicit guidance to read via file tools, with an honest "say so if file tools are off". **Document content blocks: VERIFIED and wired, 2026-08-02.** The earlier probe had hit an unauthenticated CLI — and note *why* that was not evidence either way: the CLI **parses an unsupported block shape without complaint**, so "it did not crash" proved nothing; only a reply that demonstrably read the file does. Re-run against CLI 2.1.220 with a hand-built one-page PDF containing a unique token: the model returned that exact token, `is_error: false`, `permission_denials: []`. So PDFs now ride inline as base64 `document` blocks on the same path as images (`_context_blocks`, renamed from `_image_context_blocks`), capped at **8 MB** — beyond that they stay path-only and the block text says so, because a PDF big enough to need more is better read from disk than re-inlined into every subsequent turn's context. The `<user-attachments>` guidance changed accordingly: "shown to you inline — read them directly" instead of pointing at the path.
- **Composer attachments, half (a) (#15a, 2026-07-31)** — an Attach chip next to Pins (same "context riding the next message" category) opens a **native QFileDialog**; picked files are validated and staged through the same media_staging rules (one pseudo-proposal dir per file, so chips remove individually; the startup sweep is the backstop), the chips row shows name/kind/size (paths never reach the UI), and at send the message gains a `<user-attachments>` block listing the staged **paths** with guidance to use `propose_note`'s `media[]` / `store_media_asset`. Files SURVIVE the send (the agent may propose a turn later); new chat discards them. **Deliberately not built yet** *(at the time — superseded the next day by the #15 entry above, which landed both)*: paste/drag-drop (file bytes would have to cross the webview bridge — chunking design needed) and half (b), images/PDFs as agent *context* (multimodal stream-json input blocks; PDF document-block support still unverified) — recorded here, task re-scoped.
- **Grading beyond Again (#16, 2026-08-01)** — closes P5 and, with it, the whole capability backlog. `fail_cards_now` already took a *list* of card IDs with a 50-per-operation cap, so bulk grading by ID was never the gap; the gap was that the vendored `safe_collection_operations` core could only record **Again**, and its README forbids local edits ("update it from a reviewed, pinned upstream commit"). So this landed upstream first.
  - **Upstream (`anki-addon-safe-collection-operations` 0.1.0 → 0.2.0, commit `03b5286`)**: a new `grade_cards_now(col, targets, rating=…)`, with `fail_cards_now` retained as the same operation pinned to Again — the name every existing transport advertises, so no transport broke. `Rating.from_value` accepts the enum, its integer, or its name, giving every transport one vocabulary. **The rating is the only thing that varies**: every guarantee the operation makes is about what must *not* change around the write, so the revlog-count, `reps + 1`, and hidden-queue postconditions are untouched and rating-independent, and the preliminary `Easy` on a preview card stays Easy whatever was requested (it sends the card home; it is not a grade anyone chose). Validation got *stricter*, not looser: an out-of-range or unparseable rating is refused before the backend is touched, and a transport call that omits `rating` is refused rather than defaulted to Again — a grading call that does not say what it records should not run. `api_version` stays 1; this is additive.
  - **A test double had been hiding the shape of the change.** Both repos' `FakeBackend.grade_now` bumped `reps` only for Again — indistinguishable from correct while Again was the only reachable rating, and immediately wrong once it was not. Both now count a review for every real rating, matching what the scheduler was *observed* to do.
  - **Verified against a real throwaway collection, not just the fakes**: each rating writes its correct revlog ease (Anki stores 1-4 against our 0-3 enum), each counts exactly one review, `fail_cards_now` still writes Again, suspension survives a non-Again rating, and a rejected rating writes nothing at all. The GUI smoke (71 checks) re-runs that through CWYC's own confirmation path into the re-vendored core.
  - **CWYC side**: `GradingRequest.rating` rides the payload; the rating is parsed at *submit* so a bad one is refused before a confirmation card is ever shown, rather than at apply time with the user's click already spent; warnings name the rating being recorded; and the card stops saying "Mark cards wrong" for a Good or Easy review — those are real reviews, not a verdict. The tool description says plainly that this writes real review history and must reflect a judgement the user actually made, never a guess at how well they would have done.
  - **Deliberately still refused: grading by SEARCH.** The core addresses exact card IDs and verifies stable note GUIDs immediately before the write (SAFETY.md); recording reviews at query scale is falsifying a study history, and no amount of convenience earns that. Bulk by explicit ID, which is what a grader actually produces, was already supported and remains the whole surface.
- **FSRS compute + preferences (#13/#14, 2026-08-01)** — we could already *write* FSRS `params` via `set_deck_options` but never *compute* them, so the one number that actually matters was the one the user had to fetch by hand. Four read-only tools — `fsrs_optimize`, `fsrs_evaluate`, `fsrs_simulate`, `fsrs_optimal_retention` — plus `get_preferences` / `set_preferences`. Applying any FSRS result stays `set_deck_options`, i.e. the normal proposal flow: computing is a read, changing your scheduling is a reviewed write.
  - **A new execution lane: `ToolSpec.long_running`.** FSRS optimization is seconds-to-minutes of pure compute. Every other tool marshals onto the Qt main thread via `run_on_main` with a **15s timeout** — which optimization both blows and freezes Anki through. Running it on a thread of our own is forbidden (SAFETY.md hazard 19: a stray thread touching `mw.col` poisons the backend mutex). The answer is Anki's own single collection worker: `long_running` tools go through `QueryOp(...).with_progress(label).run_in_background()`, with the MCP thread blocking on an Event up to `LONG_TOOL_TIMEOUT_S` (900s) and a timeout message that tells the agent to narrow its `search`.
  - **Every input is range-checked in Python before it crosses into Rust, and that is a safety barrier rather than politeness.** Probing found that a `SimulateFsrsReviewRequest` left at its *protobuf defaults* panics the backend (`min > max, or either was NaN`) — and a panic poisons the mutex and **kills the collection for the whole process**. So all 15 fields are populated explicitly and range-checked (`LIMITS`), `params` must be 17/19/21 finite floats, `easy_days_percentages` exactly 7 values in 0–1, and searches are validated with `find_cards` first. With nothing to simulate from we refuse in Python rather than send the zero-vector.
  - **Two sibling calls disagree about how to report the same failure.** `evaluate_params` raises `InvalidInput: Insufficient review history`; `compute_fsrs_params` returns an **empty parameter list** and no error. Passing that through would have invited the agent to propose `params: []` — silently resetting a preset's scheduling while reporting success. Both are normalised to one loud error here.
  - **And "enough reviews" is not "enough FSRS items."** Measured, not assumed: a fixture with **3080 review rows** produced `fsrs_items: 0` and zero params, because FSRS anchors each item on a card's first *learning* review and every row was review-type; flipping the first entry of each card to learning-type yielded **2860 items and 21 params from the same 3080 rows**. So the error distinguishes "no usable sequences (history imported without learning entries?)" from "only N usable reviews" — a user with thousands of reviews and zero items deserves to be told which one failed.
  - **`backups.*` is readable but deliberately not settable.** Backup retention is the safety net every destructive proposal in this add-on invokes by name ("a backup is taken first"); an assistant quietly lowering it would weaken the guarantee it had just made. Attempting it returns that reasoning rather than a bare "unknown key". `get_preferences` also reports `ignore_accents_in_search` in plain words, because it changes what the agent's *own* `search_notes` calls match — a search that finds nothing may just be this setting.
  - 19 real-Anki tests (throwaway collection per test) + a GUI smoke check (now **71**) asserting the four FSRS tools are `long_running` and read-only, that an empty simulate request is refused *before* reaching Rust, and that a preference change applies and reverts through the real protobuf.
- **Composer controls overflowed instead of shrinking (2026-08-01; structural fix 2026-08-05)** — a real-Anki capture of the dock showed the model chip printed **on top of** the permission-mode chip ("Propose" with "Default model" over it) and running past the composer's right edge. The first fix (`min-width: 0` on each popover wrapper) stopped literal overlap, but later dogfood showed the deeper problem: passive context, Pins, Attach, a sporadic learning CTA, collection permissions, computer tools, model, and Send still competed through three independently wrapping flex groups. At realistic Retina-dock CSS widths, labels ellipsized despite abundant physical screen space and the wordy permission ladder was clipped. Final structure: remove passive context chrome; move the learning CTA above the input; combine the two independent permission axes behind one amber-aware **Access · collection/computer** trigger; and make the toolbar a three-area grid (accessories / Access / model+Send), with an explicit two-row breakpoint at ≤520 CSS px. The Access panel shows a two-axis overview, then one compact label-only choice list plus one shared description; width clamps to `min(360px, 100vw - 24px)` and its list scrolls inside the smart above/below panel. Follow-up dogfood on the same day found two narrower failures: the common `Opus · medium` status still ellipsized, so closed-chip tokens are now compact (`Opus · Med`, with full names in the menu/tooltip); and hovering the final Access choices flickered because changing description height moved an upward-anchored list under the pointer, so the shared description now reserves a fixed three-line footprint.
- **Note-type write path (#7, 2026-08-01)** — the family `CAPABILITY_GAPS.md` calls "our biggest asymmetry": we had been returning template source and CSS verbatim since 2026-07-23 so the agent could *debug* rendering, and it could then diagnose a broken template and not propose the fix. Seven tools, all proposal-gated, all through `_apply_write`: `set_note_type_styling`, `set_card_template` (front and/or back source), `manage_note_type_fields` (add/rename/reposition/remove), `manage_card_templates` (add/rename/reposition/remove), `create_note_type` (clone), `change_note_type` (mapped by NAME, not position), and `remove_empty_cards` — the last one mandatory the moment templates are writable, since Anki offers no other way to delete a card whose front renders blank.
  - **Everything here was probed against a throwaway collection on 25.x before it was designed**, and the table in `NOTE_TYPE_OPS` records the results rather than the manual's version: `edit qfmt`/`edit css`/`rename field` leave the schema alone; `add field`/`reposition field`/`add template`/`remove template`/`change note type` bump it (full upload next sync); `add template` creates a card per note, `remove template` destroys them.
  - **The finding that shaped the whole design: removing a field makes Anki SILENTLY REWRITE every template that referenced it**, remapping the reference to a different field by ordinal. A `{{#Extra}}c:{{Extra}}{{/Extra}}` front came back as a bare `c:{{Front}}` — no longer conditional — and Anki generated a card on every note. (When the rewrite instead makes two fronts identical, Anki refuses the entire update with `CardTypeError`, which we surface verbatim rather than as a traceback.) Nothing in Anki's own UI tells you either happened. So the accept path **measures** the before/after card and note counts and the template sources, and reports what ACTUALLY happened as an `applied: …` line on the resolved card, instead of asserting a prediction Anki is entitled to disagree with (`lenient_cards=True`; `Expectation(changes_schema=True)` waives the scm check this family must trip).
  - **Revertibility is now a per-proposal answer, not a per-kind one** — the first family where the same tool is undoable or not depending on its arguments (rename a field vs remove one). `Proposal.revertible` is set at submit time, rides the payload, drives `_kind_revertible`, and makes the review card show a red "Cannot be undone from here — a backup is taken first" line **before** the click. The revertible ops undo by restoring the note type dict snapshotted at apply; the destructive ones (field/template removal, conversion, empty-card deletion) destroy payload that lives *outside* that dict — note content, cards, review history — so no snapshot could bring them back, and they take a **critical** checkpoint (a failed backup aborts the write).
  - Guards worth naming: the first field is refused for removal (it is the duplicate/sort key); the last field and last template are protected; a front with no `{{Field}}` is refused with the real reason rather than Anki's ordinal-flavoured error; `change_note_type` refuses a map where two sources collapse onto one target, defaults to matching **by name** (a positional default silently moves content between unrelated fields), and names every dropped field and template as a destruction warning; `remove_empty_cards` **recomputes** at accept and keeps any card that stopped being empty while the proposal sat pending; every card leads with the blast radius in notes and decks.
  - **Test posture is deliberately different from every other family.** The others are unit-tested against `FakeCol`; this one must not be, because a fake would only ever encode what we already believed — and what we believed was wrong twice. `tests/test_note_types_real_anki.py` (20 tests) drives the real `anki` library against a throwaway collection per test, skipping where the lib is absent (it is not a runtime dependency — the add-on runs inside Anki). The always-on lane is the Docker GUI smoke (now **70 checks**), which additionally proves the parts a library test cannot: the card renders its irreversibility line and correct labels in a live dock, and a destructive op really takes a **critical** checkpoint. That smoke also caught a wrong assertion of mine — I asserted the backup *file count* must rise, but `create_backup(force=True)` legitimately declines to write a new file when nothing changed since the last one, which the add-on already reads correctly as "a good backup exists".
- **Remaining deferrals closed (#24, 2026-08-01)** — four items DESIGN.md recorded as not-done, none of which had ever been in the backlog:
  - **(a) Unchanged fields collapsed** — the other half of the field-diff promise (§9's task-#31 note). Two shapes, because "unchanged" means opposite things per kind: a **create** card collapses the note type's **empty** fields behind an `N empty fields` disclosure (a 12-field note type with 3 values filled used to render nine empty boxes around the content under review; still editable once opened, and a lone empty field stays inline — a disclosure costs a click to save one line). An **edit** card gains the opposite: `Proposal.context_fields` carries the note's **untouched** fields, collapsed by default, read-only, never part of the write. The diff still shows only what changes, but "does this new value duplicate something already on the note?" was unanswerable without them. Partitioned on the *proposed* value, not the live draft, so a field cannot hop between sections while you type into it.
  - **(b) Media on edits** — proposal media had been create-kind only, so an edit could not attach TTS audio or a diagram to an existing note, and `_attach_preview_media` (#25's existing-`[sound:]` player strip) was never called for edits either. `propose_note_edit` now takes `media[]` through the same staging path, and the legacy card (the route that renders edits — the shared `InteractionCard` and its schema-1.1 player strip are create-only) grew its own `AudioStrip`. One real subtlety the create path does not have: an edit writes the **reviewer's** values, not `proposal.fields`, so `_import_staged_media` now **returns its renames** and `_accept_edit` rewrites the markers in the values it is about to write — otherwise a rename-on-import would silently write a marker pointing at a filename that no longer exists. Preview media is scanned over the **changed** values only; the whole note's audio would drown the diff. Staging is still *not* discarded on reject/supersede (`restore()` can bring a proposal back and its accept would need the files) — the startup sweep frees what never resolves.
  - **(c) Auto-denial detection** — documented 2026-07-14 with the stream-json signals verified against CLI 2.1.208 and explicitly "not built". Now built: the end-of-turn `result` message's `permission_denials[]` becomes a `PermissionDenied` event (`{tool, detail}` per entry, malformed entries skipped rather than allowed to break the turn's `Done`) and renders as its own banner. Deliberately **not** an error banner: a blocked call ends the turn `subtype: "success"` and the model just works around it in prose, so the reply reads as the assistant declining when in fact the mode the user chose stopped it — the banner says which mode, and nothing "failed".
  - **(d) Per-field reject with a comment** — per-field ACCEPT has been in the protocol since M2 (`accepted_fields`), but the card always sent every field, so "apply the Front fix, leave Back alone" was unreachable. Each changed field on an edit card now carries a Skip toggle; skipped fields are struck through, excluded from `accepted_fields`, and reveal a one-line "why?" box whose text rides the `reviewed` observation as `declined_field_comments` into the learning store — a bare `declined_fields` list teaches nothing, the reason is the whole point. Accept is disabled (with a reason) when every field is skipped and no tag change remains, rather than letting the click bounce off Python's "nothing selected to apply".
  - GUI smoke (69 checks) proves the parts only real Anki can: an edit's staged wav **decodes** in QtWebEngine, the context block renders collapsed reading "1 unchanged field", and after a Skip-with-comment accept the skipped field still holds its original value in the collection. The first draft of that check asserted context fields on a Basic note with **both** fields edited — where there are none — and the smoke caught it.
- **Media beyond audio (#10, 2026-07-31)** — the proposal `media[]` staging path (§8 / task #21) now takes **images** (png/jpg/jpeg/gif/webp/svg/avif) and **video** (mp4/webm) alongside audio; kind derives from the extension, images are referenced as `<img src>` (audio/video keep Anki's `[sound:...]`), the unreferenced-attachment check and the rename-on-import rewrite both understand `<img src>` (`media_references` / `rewrite_media_markers`), and the review card previews visuals in a CWYC-owned strip (the interaction-schema `MediaAttachment` stays audio-only by design — visuals deliberately do not widen a schema shared with other hosts). Plus **`store_media_asset`**: non-note assets (fonts, shared template images, stylesheets) into collection.media via one confirmation with an inline preview; imported through `col.media.add_file`, revert moves the file to Anki's media trash (recoverable), `_`-prefixed names recommended for template assets (protected from unused-media cleanup). Image Occlusion's shape-data format remains out of scope.
- **Batched filtered-deck actions (#27 quick win, 2026-07-31)** — `filtered_deck_action` accepts one `deck`, a `decks` list, or a `pattern` glob over filtered-deck names; ONE review card for the whole batch, per-deck gather counts as explicit outcomes on the resolved card (never one opaque success), a normal deck in an explicit list is a loud error while a pattern silently skips what it sweeps up. This directly fixes the lived 50-rebuilds-50-cards case (2026-07-23). **The generic batch machinery (#27 proper) shipped 2026-07-31**, right after this was written: `BATCHABLE_OPS` declares a risk class, a revertibility statement and a validator per op; a change set composes them through `_internal_apply_op`, which drives each item's own real submit+accept path under a quiet flag (`self._quiet`) so a batch item cannot take a shortcut around the chokepoint its standalone tool uses; risk and revert text show on every row up front; and the review card carries per-item include checkboxes whose unchecked indices are excluded at accept. The caution recorded here — "not as a tail-of-session change to the safety-critical apply path" — was right, and is why composition reuses the existing paths instead of adding a second one.
- **Permission UX (#26, 2026-07-31; unified 2026-08-05)** — collection permission and harness environment power remain separate enforcement axes, but no longer masquerade as two unexplained neighboring chips. One **Access · collection/computer** trigger opens an overview with **Anki collection** and **Computer tools** rows, then drills into either axis. Choice lists are compact labels only; one shared description reflects the last selected/focused/hovered item. Hover previews are sticky until another item is entered—leaving a row no longer resets the copy while the pointer crosses the inter-row gap. The collection detail retains "What happens under X?" and its per-class matrix (Reads / New notes / Note edits & bulk sweeps / Cards, tags & scheduling / routine Decks & structure / Full-sync changes / Grading / Destructive deletes / Skill updates). The matrix presents policy enforced in Python. **Trusted writes means “roam freely, stop at the cliffs”:** routine note, bulk, scheduling, and deck writes auto-apply within the session budget, while destructive deletes, non-revertible note-type operations, changes known to force a one-way/full sync, and skill changes remain review cards. **Full collection** is the explicit next tier: destructive and full-sync collection operations also auto-apply within the same budget and with critical backups; skill changes still remain review cards because they alter future behavior rather than the current collection. `Proposal.requires_confirmation` is enforced by `_finish_submission`; the UI is not the boundary.
- **Large preview window (#2, 2026-07-31, route B as decided 2026-07-23)** — an expand affordance on the proposal card's flip preview posts `proposal_preview_window` (with the in-progress draft fields, so the big window matches what is being edited); `render_for_window` reuses the same ephemeral renderers as the in-card preview, and `preview_window.py` shows a resizable QDialog + AnkiWebView with Before/After (edits, opens on After) or Front/Back (creations) face buttons, persisted geometry (`cwycProposalPreview`), night-mode-mirrored srcdoc identical in shape to the in-card iframes, and a replace-on-reopen singleton. Route A (subclassing aqt's Previewer) stays rejected: its render path reloads the note from the DB and breaks for create proposals.
- **Per-deck limits (#25, 2026-07-31)** — `set_deck_limits`: today-only overrides (`new_limit_today` / `review_limit_today`, stored on the DECK as `{limit, today}` and self-expiring at rollover) plus permanent per-deck caps (`new_limit` / `review_limit`); `-1` clears. These live on the deck object, which is why `set_deck_options` structurally couldn't reach them. The Study Triage headline action ("Set Today's New Cards to 0") and Custom Study's increase-limits, as one proposal-gated deck op with per-deck×field samples and prior-value revert. Cascade decision: `include_subdecks` (default false) — documented v3 reality: a **parent's limit caps its whole subtree**, so zeroing the parent alone silences it, while *raising* limits may need the cascade because each child's own limit still binds; filtered subdecks (no limits) are skipped with a note. `get_deck_info` now reads the four values back, marking today-entries active/expired.
- **Scheduling writes (#6, 2026-07-31)** — `set_due_date` (`n` / `n-m` / trailing `!` = also set interval; the backlog-spreading and push-past-exam tool), `forget_cards` (reset to new; `restore_position` / `reset_counts`), `reposition_new_cards` (`starting_from` / `step_size` / `randomize` / `shift_existing`, new cards only). Same selection contract (one of `card_ids`/`query`); these rewrite scheduling in bulk, so the proposal card gets a **louder diff**: per-sample before/after scheduling lines ("new · position 12" → "review · due in 16-20d") plus the count, and honest warnings (`!` overwrites intervals; N new cards convert; `shift_existing` renumbers bystanders that undo won't restore). Revert is an **exact update_card restore** of every capturable field (type/queue/due/ivl/factor/left/odue/reps/lapses/custom_data/memory_state, captured at apply as live objects — the ledger is in-process) — stronger than Anki's own UI, which calls the new→review conversion not practically reversible; probed on 25.09 that a converted card round-trips exactly. Reposition claims only the genuinely-stamped new cards in the written-rows invariant.
- **Card-state ops (#3, 2026-07-30)** — `suspend_cards` / `unsuspend_cards` / `bury_cards` / `unbury_cards` / `set_card_flag` (flag 0-7, 0 clears). One shared submit path (`submit_card_state`): each takes **exactly one** of `card_ids` (≤2000, from find_cards) or an Anki `query`; one bulk proposal card with an honest count, no-op accounting ("N already suspended"), and a filtered-deck warning (suspend/bury return such cards to their home deck; undo cannot re-add them to the filtered deck — same class of one-way loss as move_cards' FSRS wipe, surfaced the same way). Prior state (queue / 3-bit flag) is captured per card **at apply time**, not submit; revert is drift-aware — it reads each card's *current* queue and issues only the transitions still needed (`_restore_card_queues`), because manual buries expire at rollover and the user can flip states in the Browser between apply and revert; a blind reverse-op would corrupt cards that no longer need it. `bury_cards` cross-references `defer_card` for the single-card "not now" during review (defer is tray-tracked). Flags round out the read side that already existed (`user_flag` in card summaries, `flag:N` searches).

Permission modes (per-profile setting + per-session override in the composer Access panel):
...
- **Trusted writes**: routine collection changes apply immediately within a
  session budget; destructive/non-revertible changes, full-sync changes, and
  skill changes always stop for explicit review.
- **Full collection**: all collection changes, including destructive and
  full-sync operations, apply immediately within the same session budget;
  critical backups remain mandatory and skill changes still require review.
...
- **trusted-writes tier**: routine write tools (update/add/bulk/deck and scheduling operations) apply without proposal cards. Safeguards: backup checkpoints, every touched note ledgered + provenance-tagged, a per-session write budget that pauses for confirmation when exceeded, and explicit review for destructive/non-revertible operations, changes known to force a one-way/full sync, and skill changes.
- **full-collection tier**: crosses Trusted writes' destructive/full-sync boundary while retaining the session budget, ledger, and mandatory critical backups. Skill changes remain review-only in every tier.
- **agent-tools axis / environment power** (`agent_tools`: `sandbox` default | `acceptEdits` | `auto` | `full`; Slice 1 shipped 2026-07-13 as a binary, widened to four tiers 2026-07-14): a **separate, orthogonal axis** from the collection-write permission mode above — that axis gates *collection* writes; this one gates the CLI's own Bash/file tools. `sandbox` (default) keeps the current posture: `--disallowedTools Bash,Edit,Write,NotebookEdit`, no `--permission-mode`. `full` drops those disallows and adds `--permission-mode bypassPermissions` (auto-approve — headless `-p` has no interactive prompt, so a shell/file call would otherwise be refused). Both share the identical MCP-scoping / web / model-effort-fast flags, and switching respawns the CLI with `--resume` on the next message, exactly like a model switch. Sharp edge: card/field content is untrusted model input (§13.3), so `full` turns a malicious shared deck's prompt injection into **immediate** arbitrary code execution — there is no per-command gate. This is surfaced honestly: the constraint line in `--append-system-prompt` and the agent-home `CLAUDE.md` are both **conditional** on the mode (they never claim shell is off when it is on, or vice-versa), and the unified Access trigger keeps an amber warning whenever Computer tools are non-sandbox; the panel retains a "What's the risk?" modal spelling out the injection danger. Claude Code's built-in circuit breaker (`rm -rf /`, `rm -rf ~`) and deny rules still apply. **Honest note:** `full` makes the collection-write safety chokepoint (proposals/review) an *informed, bypassable default* — a shell can route around it — so the mode is documented as trust-scoped and is not sticky-safe on shared collections.
...
2. **Full-skill tier**: user drops a complete skill directory (SKILL.md + references + scripts). Harness backends mount it as a real skill; scripts are runnable only when the selected computer-access mode permits them.
...
worked out with the user (for example, a repeatable image-cleanup pipeline;
nothing in the mechanism is task-specific) as a
...
**Review decisions return to the agent (2026-09-04).** Accept/Reject clicks are
host UI actions, not Claude messages. A successful manual decision is now queued
as compact structured context and attached to the user's next real message,
including the actual accepted values and skipped fields for partial edits. This
keeps the agent's model of the collection accurate without triggering an
unsolicited reply when the user only clicks a button. Session-only slash
commands do not consume the queue; a new chat or history switch discards it.
Auto-applied proposals need no duplicate notice because their tool result
already reports the outcome to the agent.
...
**Open a proposal in a large preview window (SPEC — SHIPPED 2026-07-31 as task
#2; see §5. The spec below is kept as the record of the route decision).** The in-card preview is a small fixed square; a complex card needs
...
**Template & note-type writing (task #33, SPEC — SHIPPED 2026-08-01 as task #7;
see §5, which also records the probed Anki semantics that changed the design.
The spec below is kept as the record of the gating decision).** Today the agent
...
**Composer attachments & multimodal input (task #28, SPEC — SHIPPED 2026-07-31
/ 2026-08-01 as tasks #15a and #15; see §5. PDF *document* content blocks were the last
unverified piece and are now verified and wired — see §5, 2026-08-02).**
...
- **States**: the chat itself always starts empty — **no assistant preamble, no auto-greeting**; the conversation begins when the user sends the first message. Card context is assembled lazily at first send; collection structure stays tool-on-demand rather than an injected overview. The CLI process pre-warms when the chat gains focus, so spawn latency is hidden well before the first send (a hidden/unfocused dock spawns nothing). Context pushes remain internal UI state for card-vs-collection suggestions, but no passive context chip is rendered: on the deck browser it was noise, and in review the card itself already supplies the visible task context.
...
- **Chat history**: sessions persisted per-profile as **our own event-sourced JSON transcripts** in `user_files/` — every rendered event (user/assistant messages, tool calls + results, proposal cards and their outcomes, permission decisions, and internal context state) is the source of truth for display and history. Each transcript stores the backend's session id, so "continue this chat" maps to the backend's native resume (`claude --resume <id>`) while pixels come from our file. CLI session files are deliberately *not* parsed: their formats are internal and version-unstable, and they lack our UI-level events. Reopening Anki starts a fresh chat by default (the previous one is in History); the opt-in `restore_last_chat` config reopens the most recent chat automatically (same replay + `--resume` path as picking it from History).
- **Claude commands and skills (verified/fixed 2026-09-04)**: the persistent
  stream-json session dispatches Claude Code commands that are supported without
  an interactive terminal (`/compact` verified end to end), and its discovered
  skills are genuinely available. A slash invocation must be the first content
  in the outbound message, so CWYC now keeps it there and appends fresh card
  context only after skill-like invocations. Session-only commands such as
  `/compact`, `/context`, and `/usage` bypass card/decision injection entirely;
  the `compact_boundary` system event surfaces as a transient "Conversation
  compacted" confirmation instead of disappearing. Starting a new chat now
  clears all chat-scoped frontend state (ledger, pending proposal ids,
  attachments, active proposal, and stale notice), while history replay rebuilds
  its own state from recorded events.
...
- **First-run onboarding for no-CLI installs, no restart (task #19, added 2026-07-16; release wording updated 2026-08-06)**: previously a user without Claude Code got one line ("Claude Code CLI not found — using the built-in demo backend…") and nothing actionable. `ChatController._build_backend()` now pushes a structured `{"type": "setup_needed", "platform": "darwin"|"linux"|"windows"}` event instead, and the UI renders a step-numbered setup card: (1) a platform-appropriate official install command and documentation link, (2) instructions to authenticate the official Claude Code CLI, and (3) a **Re-check** button. CWYC does not accept or store API keys. Re-check rebuilds the backend and session without restarting Anki; failure leaves the deterministic demo backend available and surfaces a PATH hint.
...
- A fake backend (`ScriptedBackend` replaying canned event streams) so UI/UX iteration never needs a live CLI and smoke tests remain deterministic.
...
- **M3 — Polish on the Claude Code backend** *(rescoped 2026-07-05; substantially landed same day)*: ✅ chat history/resume (event-sourced transcripts in `user_files/transcripts/`, history panel, replay + native `--resume`; pending proposals from an old session render as set-aside on replay); ✅ suggested questions; ✅ unified Access panel + Shift+Tab with server-side collection enforcement; ✅ internal live context state (the noisy passive chip was later removed); ✅ cost/usage display; ✅ doctor-lite panel (harness/op presence + versions on a background thread, broken-install detection, install links); ✅ **open-in-Claude-Code** (header button hands the chat to a terminal Claude Code via `--resume` in agent-home; `agent-home/.mcp.json` is rewritten each run with the MCP url+token — 0600 — so the terminal instance keeps the anki tools *and* gains Claude Code's full toolset; since 2026-07-13 the handoff also **carries the dock's agent settings** — `--model`, `--effort`, `--settings '{"fastMode":true}'`, and `--permission-mode bypassPermissions` when agent-tools is full — while the collection-write permission mode needs no flag because our MCP server enforces the live config; the GUI deep link accepts none of these parameters, so the Desktop path can't carry them). ✅ **ask-each-read mode** (opt-in for untrusted shared decks): every read tool call blocks its MCP thread on an inline Allow/Deny chip via an ApprovalBroker; teardown unblocks-as-deny; writes are not double-gated — proposals already review them. **Reworked 2026-07-23 (dogfood):** the original 120s block outlived the MCP *client's* own tool timeout, so slow-to-answer prompts got their calls killed out from under us and the agent — handed a bare timeout — told the user their collection was busy mid-sync while four prompts sat unanswered on screen. The call now waits only `APPROVAL_GRACE_S` (45s, comfortably under any client timeout) and otherwise returns **PENDING**, which is explicitly *not* a denial: the caller raises "approval pending, this call did NOT run, mention it once then drop it", the chip stays answerable, and the answer is remembered once (`DECISION_TTL_S`, 5 min, one-shot) so the agent's next attempt consumes it without re-prompting. Retries of an unanswered call re-use the live chip instead of stacking duplicates, and an answer that arrives after the call gave up auto-posts a "go ahead" message so the work actually resumes. **Expiry (2026-07-27):** a prompt left unanswered dies after `approval_timeout_minutes` (default **5**, `0` = never) and resolves as "Expired, no answer" — never "Denied", since nothing was refused. The chip carries its wall-clock deadline and counts down, so an abandoned prompt reads as abandoned instead of looking answerable forever; without this the agent kept re-raising requests the user had given up on hours earlier. ✅ **Open in GUI Claude Code**: the button is now a chooser (Desktop app / Terminal). Desktop uses the documented `claude://code/new?folder=…&q=…` deep link — the app cannot resume a session by id yet (upstream feature requests open), so the handoff opens a new session in agent-home with a prompt pointing at this chat's transcript file, which the GUI agent reads for context; the terminal path keeps true `--resume`. Config dialog dropped in favor of in-dock panels.
...
- **M4 — Distribution & polish** *(v0.1.0, 2026-08-06)*: deterministic `.ankiaddon` packaging, GitHub and AnkiWeb release metadata, Claude Code setup guidance, privacy/security disclosure, compatibility QA, and public-preview positioning.
...
1. **CLI requirement vs. off-the-shelf simplicity** — the central tension. v0.1.0 knowingly serves users comfortable installing and authenticating Claude Code; future Codex/Pi adapters may broaden that audience.
...
11. **Usage visibility — resolved**: token/context usage is visible. Dollar cost is intentionally hidden because the supported harness-authenticated backend reports a notional API-equivalent amount rather than a CWYC charge.
...
14. **M2 implementation notes (2026-07-03; scheduler qualification 2026-07-24).** ProposalManager is the single content/deck write path; native review writes use the separate narrow GradingManager described in §17.6. Agent tools submit to those managers, their webview cards accept/reject through them, and everything runs on Anki's main thread (tool calls are already marshaled there). The proposal staleness guard snapshots changed fields at *submit* time and re-checks at *accept* time; on mismatch it refreshes the baseline and re-renders for re-review instead of applying. Per-field acceptance ships in v1; granular per-field reject-with-comment did not. Previews render via `Note.ephemeral_card()` + `render_output()` with model CSS in a sandboxed iframe. Demo/smoke coverage routes ScriptedBackend proposal events into the real ProposalManager, and the Docker GUI smoke tests both proposal and grading managers against real Anki. The session ledger is in-memory per chat session. Keyboard review: `Cmd+Enter` accept / `Cmd+Backspace` reject target the focused (else earliest) pending proposal, `Cmd+Up/Down` cycle pending proposals. Auto-accept: creations and native grading use separate counters under the per-session cap; edits remain manual.
...
decks. Every operation is represented as a `deck_op` proposal with
plain-language sample lines, flowing through the same `_finish_submission`
machinery as the bulk ops (routine operations apply directly under
Trusted writes within the write budget; Full collection also directly applies
destructive/full-sync operations; deck ops cost 1 budget unit).
...
1. **Fail the atomics the LLM judged wrong** = record honest **Again**
   ratings through the pinned, vendored
   **Safe Collection Operations** core. The core uses Anki's native Browser
   **Grade Now** backend operation (`col._backend.grade_now`), which is built
   to answer arbitrary cards outside today's queue: Anki derives the current
   scheduling state, records the revlog, and lets the active scheduler/FSRS
   choose the next state. No direct card-row or due-date writes. The shared
   core and CWYC policy integration are proven against real Anki 25.09:
   ...
   Existing suspension, manual burial, and scheduler burial are restored
   natively *after* the Again, so the failure remains in review history while
   the card stays hidden. Native leech suspension takes precedence over prior
   burial and is reported separately. The result always offers a distinct
   `make_cards_available` operation; that removes only suspension/burial and
   ...
   Trusted writes, and Full collection apply under their session budgets;
   ...
   card is graded, CWYC resets the reviewer so a stale in-memory card cannot be
   answered twice.
