...
- **Chrome parity (#23, 2026-08-01)** — the documented classic-UI gaps closed: (a) the **context chip** ("card in X" / collection default) renders again from controller's `context` pushes, in the composer's context group; (b) **suggested-question ghost text** — rotating, context-aware placeholders (card vs collection pools) with **Tab-to-accept** on an empty composer, gated by the `suggested_questions` config; (c) the **bulk bar** — two or more pending proposals surface "N proposals pending · Reject all / Accept all" above the composer (bare accepts; Python falls back to each proposal's own values); (d) the **learning nudge chip** (`learning` pushes) — "✎ Review N edits" kicks off the skill-review chat; (e) **friendly tool labels** — raw MCP names map to human phrases ("Searched your cards"), unknown names prettify, plumbing (ToolSearch, StructuredOutput, TodoWrite) renders nothing, and the raw name stays inside the expanded details so debugging loses nothing. The composer context group now shrinks/wraps (it grew from two chips to five).
...
- **Composer chips overflowed instead of shrinking (2026-08-01, found by running it)** — a real-Anki capture of the dock showed the model chip printed **on top of** the permission-mode chip ("Propose" with "Default model" over it) and running past the composer's right edge. `.cwyc-ctl`, the popover wrapper around each chip, is a flex item with the default `min-width: auto`, so it refused to shrink below its content: the right-hand group got squeezed to 58px while the chip inside stayed 89px and overflowed both ways. The existing `max-width: 168px` ellipsis rule could not help — 168px is a cap, not a floor, and nothing was letting the box get smaller in the first place. Fixed with `min-width: 0` on the wrapper and a shrinkable chip inside it; verified at 340px (below the 360px dock floor) with no pair of composer controls overlapping, then re-verified in a real Anki capture. Worth recording as a method note: this survived every unit test, every smoke check, and the browser dev preview at its usual width — it took *looking at the running app* at a realistic dock width.
...
- **Permission UX (#26, 2026-07-31)** — the mode picker is now a visual **ladder** (most restrictive → most permissive, rail + dots) with hints that name **operation classes** instead of "cards"/"notes", plus a "What happens under X?" disclosure rendering a per-class matrix for the current mode (Reads / New notes / Note edits & bulk sweeps / Cards, tags & scheduling / Decks & structure / Grading / Deletes / Skill updates) and a pointer to the orthogonal agent-tools axis. Presentation only — the carve-outs stay hardcoded in Python; the matrix just stops them being invisible. **Fact correction recorded:** the task's claim that deck & structure ops never auto-apply under trusted-writes was WRONG — `_deck_op_proposal` has flowed through `_finish_submission`'s trusted auto-apply since deck ops were introduced (028aedd, 2026-07-06), verified live; only deletes (always confirmed + backup, and only offered under trusted) and skill updates (always confirmed) are exempt. The copy now describes that reality.
