...
13. **Resolved 2026-07-02**: chats lead with nothing — no preamble; session and context assembly start lazily at the user's first message, in both reviewer and deck browser. Default chords proposed in §9 (`Cmd+J` toggle, `Cmd+Shift+J` new chat, `Esc` to leave) pending user sign-off. Chat UX copies Claude Code / ChatGPT conventions verbatim (§9). Transcripts: own event-sourced JSON with embedded backend session id for native resume (§9) — CLI session files not parsed.
14. **Open — display name.** Candidates with trade-offs (all paired with a functional AnkiWeb subtitle like "— AI chat dock for your reviews" for search discoverability): **Deskmate** (classmate-beside-you metaphor, matches a sidebar, uncrowded, icon-able — current recommendation); **Marginalia** (chat in the margins of studying; most distinctive, but obscure and weak for search); **Sidekick** (legible sidebar pun, but the most cliché and collided AI name); **Study Chat** (max discoverability, zero personality); **Socratic** (best concept fit for dialogue-about-the-card, but pretentious and Google-owned app namespace). Avoid "Copilot" (trademark) and model-vendor names (backend-plural by design). The user has their own candidate, deliberately unshared yet. An SVG icon will be designed once the name lands — keep the name icon-able.
