...
- **Permission UX (#26, 2026-07-31; unified 2026-08-05)** — collection permission and harness environment power remain separate enforcement axes, but no longer masquerade as two unexplained neighboring chips. One **Access · collection/computer** trigger opens an overview with **Anki collection** and **Computer tools** rows, then drills into either axis. Choice lists are compact labels only; one shared description reflects the selected/focused item, avoiding the clipped wall of repeated hints seen in the old collection ladder. The collection detail retains "What happens under X?" and its per-class matrix (Reads / New notes / Note edits & bulk sweeps / Cards, tags & scheduling / routine Decks & structure / Full-sync changes / Grading / Destructive deletes / Skill updates). The matrix presents policy enforced in Python. **Trusted writes now means “roam freely, stop at the cliffs”:** routine note, bulk, scheduling, and deck writes auto-apply within the session budget, while destructive deletes, non-revertible note-type operations, changes known to force a one-way/full sync, and skill changes always remain review cards. `Proposal.requires_confirmation` is set by the submit path and `_finish_submission` enforces it server-side; the UI is not the boundary. Full-sync examples include structural note-type operations and deleting an options preset. This refines the earlier behavior, under which routine deck operations correctly auto-applied but full-sync note-type/preset changes could also auto-apply.
...
- **trusted-writes tier** (new mode): routine write tools (update/add/bulk/deck and scheduling operations) apply without proposal cards. Safeguards: backup checkpoints, every touched note ledgered + provenance-tagged, a per-session write budget that pauses for confirmation when exceeded, and explicit review for destructive/non-revertible operations, changes known to force a one-way/full sync, and skill changes.
...
trusted-writes within the write budget; destructive or full-sync operations
remain review cards; deck ops cost 1 budget unit).
