...
- **Composer attachments finished (#15, 2026-08-01)** — the deferred halves landed. **Transports:** OS **drag-drop** is intercepted at the Qt layer (event filter on the dock webview + its render child): file URLS become paths, so no bytes cross the bridge, and non-file drags (text) pass through untouched; **paste** is the one transport where bytes legitimately cross (a clipboard screenshot has no path) — image clipboard items go over the bridge as a single data-URL message, the same route Anki's own editor paste uses, capped client-side (8 MB, with a visible notice) and again at staging. **Agent context (#15b):** attached images ≤3.75 MB ride the send as base64 **image content blocks** appended after the text (backend `send` grew an `extra_blocks` param through the ChatSession protocol; the scripted backend accepts and ignores them), and the `<user-attachments>` text says which images are inline vs path-only. **PDFs** stage as a new `document` kind — accepted by the composer and `store_media_asset`, REJECTED by note proposals (a PDF can't render on a card) — and are handled per the task's fallback: path + explicit guidance to read via file tools, with an honest "say so if file tools are off". **Document content blocks remain unverified**: the live probe (`claude -p --input-format stream-json` with an image + document block, scratchpad `probe-msg.jsonl`) hit an expired CLI OAuth session — the CLI *parsed* the message shape without complaint but never reached the API; re-run the probe after `claude login` before wiring document blocks into the send path.
...
/ 2026-08-01 as tasks #15a and #15; see §5. One piece is genuinely still
unverified: PDF *document* content blocks, blocked on an expired CLI OAuth
session — re-run the probe after `claude login`).**
