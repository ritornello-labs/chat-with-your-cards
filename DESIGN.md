...
**MCP scoping — restrictive by default, opt-in to widen (decided 2026-07-10).** The dock spawns the CLI with `--strict-mcp-config` and a config containing only our `anki` server, so the agent sees *our* tools and nothing else — not the user's own Claude Code MCP servers. This is deliberate isolation: card/field content is untrusted model input (§13.3), so silently wiring every MCP server the user configured for coding into a context that also ingests untrusted card text is an exfiltration surface (a malicious shared deck could steer the agent into calling their filesystem/GitHub/etc. server). Default safe, same posture as the deliberately-unshipped unrestricted tier (§5). The user may opt in to widen the set, all buildable on flags already in use: **add dock-specific servers** (extra entries in our `--mcp-config`), **inherit their Claude Code servers** (drop `--strict-mcp-config`), and **disable any individual server, ours or theirs** (`--disallowedTools mcp__<name>`), with per-server toggles. This is a planned config surface (ships with the settings panel), not current behavior — today it is strict-only.
