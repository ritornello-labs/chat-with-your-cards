...
- **M3 — Polish on the Claude Code backend** *(rescoped again 2026-07-05: Codex and Pi adapters deferred to a later version by user decision)*: chat history/resume; suggested questions (static ghost text); permission-mode chip + Shift+Tab cycling with server-side enforcement; live context chip; per-session cost/usage display from stream-json result events; ask-each-read mode; doctor-lite (report claude CLI/op presence + versions, link install pages). Groundwork already landed: backend config accepts `codex`/`pi` (explicit not-yet notice), BYOK key plumbing, web access, skills dir, compact tools.
