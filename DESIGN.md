...
- **agent-tools axis / environment power** (`agent_tools`: `sandbox` default | `full`; Slice 1 shipped 2026-07-13): a **separate, orthogonal axis** from the collection-write permission mode above — that axis gates *collection* writes; this one gates the CLI's own Bash/file tools. `sandbox` (default) keeps the current posture: `--disallowedTools Bash,Edit,Write,NotebookEdit`, no `--permission-mode`. `full` drops those disallows and adds `--permission-mode bypassPermissions` (auto-approve — headless `-p` has no interactive prompt, so a shell/file call would otherwise be refused). Both share the identical MCP-scoping / web / model-effort-fast flags, and switching respawns the CLI with `--resume` on the next message, exactly like a model switch. Sharp edge: card/field content is untrusted model input (§13.3), so `full` turns a malicious shared deck's prompt injection into **immediate** arbitrary code execution — there is no per-command gate. This is surfaced honestly: the constraint line in `--append-system-prompt` and the agent-home `CLAUDE.md` are both **conditional** on the mode (they never claim shell is off when it is on, or vice-versa), and the dock shows a dismissable amber risk line plus a "What's the risk?" modal spelling out the injection danger. Claude Code's built-in circuit breaker (`rm -rf /`, `rm -rf ~`) and deny rules still apply. **Honest note:** `full` makes the collection-write safety chokepoint (proposals/review) an *informed, bypassable default* — a shell can route around it — so the mode is documented as trust-scoped and is not sticky-safe on shared collections.
  - **Slice 2 (not implemented):** the intermediate Claude Code permission modes (`default` / `acceptEdits` / `plan`) between sandbox and full-bypass. They need an undocumented permission-prompt-tool to surface per-command approvals in the dock, so they are deferred; the UI's "Agent tools" section is structured so they slot in as additional items. Collection-power and environment-power remain deliberately separate axes.
...
- **Context-window usage (added 2026-07-13)**: the stream's `usage` object never carries a context-WINDOW size, only cumulative per-turn token counts — `input_tokens`/`output_tokens`/`cache_creation_input_tokens`/`cache_read_input_tokens` (Anthropic API usage field names). "Context used" for a turn is approximated as `input_tokens + cache_read_tokens + cache_creation_tokens` (the size of that turn's own request, since each call resends the growing conversation as its input — no manual summing across turns). The window size itself is **hardcoded per model** in two kept-in-sync tables — `context_window_for()` in `chat_with_your_cards/backends/claude_cli.py` (unit-tested) and its TypeScript port `ui/src/contextWindow.ts` (used live by `UsageFooter.tsx`, since the footer — not Python — is what needs the ratio): **1,000,000** tokens for Opus 4.6/4.7/4.8, Sonnet 4.6/5, Fable 5, Mythos*; **200,000** for Sonnet ≤4.5, Haiku ≤4.5, and anything unrecognized (safe default). Update both tables together when a new model generation ships. The footer shows a compact `128k / 1M context` readout with a thin proportional bar (amber fill, turning the warning red past ~80% full).
